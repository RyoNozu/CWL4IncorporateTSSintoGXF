...
def extract_gene_id(attribute_column, file_type):
    ...
    if file_type == "gtf":
        ...
        return attribute_column.str.extract(r'(?:gene=|gene_id=|ID=gene-|Parent=gene-)([^;]+)')
    else:
...
def update_gtf_with_tss(gtf_file, tss_file, output_file):
    ...
    # gene_id を含まない行を確認
    missing_gene_id_rows = gtf_df[gtf_df['gene_id'].isna()]
    
    # gene_id を含まない行が存在する場合、出力
    ...
    processed_rows = gtf_df[gtf_df['gene_id'].isin(processed_gene_ids)]
    ...
    # TSSのgene_idに基づいて更新
    for target_gene_id in tss_gene_ids:
        print(f"Processing gene_id: {target_gene_id}")
        gene_group = gtf_df[gtf_df['gene_id'] == target_gene_id]
        if gene_group.empty:
            print(f"Gene ID {target_gene_id} not found in GTF/GFF file.")
            continue
        ...
        if gene.empty or cds.empty:
            print(f"Skipping gene_id {target_gene_id} due to missing gene or CDS.")
            
            # 該当する gene_id を持つ行を unchanged_gtf に追加
            for idx, row in gene_group.iterrows():
                unchanged_gtf.append({**row.to_dict(), 'original_index': idx})
                print(f"Added row for gene_id {target_gene_id} to unchanged_gtf: {row.to_dict()}")
        ...
        tss_group = tss_df[tss_df['gene_id'] == target_gene_id]
        ...
        print(f"Transcript IDs for gene_id {target_gene_id}: {transcript_ids}")

        if len(transcript_ids) == 1:
            # 1つのtranscript_idを持つ場合の処理
            transcript_id = transcript_ids[0]
            print(f"Gene ID {target_gene_id} has a single transcript_id: {transcript_id}")
            ...
            if strand == '+':
                if tss_pos < cds[cds['transcript_id'] == transcript_id]['start'].min():
                    ...
                    print(f"Updated gene start for gene_id {target_gene_id} to {tss_pos}.")
                    ...
                    print(f"Gene start for gene_id {target_gene_id} not updated (TSS position is inside CDS).")
                    # オリジナルの gene 行を updated_gtf に追加
                    original_gene = gene.iloc[0].to_dict()
                    original_gene['original_index'] = gene.index[0]
                    updated_gtf.append(original_gene)
                    print(f"Original gene entry added to updated_gtf for gene_id {target_gene_id}:")
                ...
                if not tss_group.empty:
                    ...
                    if file_type == "gtf":
                        tss_attribute = (
                            ...
                            f'tss_id "tss_id-{tss_group.iloc[0]["tss_id"]}";'
                        )
                    elif file_type == "gff":
                        tss_attribute = (
                            f'ID=tss_id-{tss_group.iloc[0]["tss_id"]};'
                            f'gene={gene.iloc[0]["gene_id"]};'
                            f'transcript_id={transcript_id};'
                        )
                    else:
                        raise ValueError("Unsupported file type. Only GTF and GFF are supported.")
                    ...
                    # TSS feature の行を作成
                    tss_feature = pd.DataFrame({
                        'seqname': [gene.iloc[0]['seqname']],
                        'source': ['TSSr'],
                        'feature': ['TSS'],
                        'start': [int(tss_pos)],
                        'end': [int(tss_pos)],  # TSS は単一の位置なので start と end は同じ
                        'score': [tss_group.iloc[0]['score']],  # tss_df の score を使用
                        'strand': [strand],
                        'frame': ['.'],
                        'attribute': [tss_attribute]
                    })
                    ...
                    # TSS feature をリストに追加
                    tss_feature_dict = {**tss_feature.iloc[0].to_dict(), 'original_index': last_cds_index}
                    updated_gtf.append(tss_feature_dict)
                
                    # デバッグ: TSS feature の情報を確認
                    print("Added TSS feature:")
                    print(tss_feature_dict)
                ...
                # five_prime_UTR_start と five_prime_UTR_end の範囲が正しい場合のみ処理
                if five_prime_utr_start <= five_prime_utr_end:
                    # updated_gtf から更新された exon 行を抽出
                    updated_exons = pd.DataFrame([entry for entry in updated_gtf if entry['feature'] == 'exon'])

                    # updated_exons が空でない場合に処理を実行
                    if not updated_exons.empty:
                        # tss_pos から CDS の start までに含まれる exon を抽出
                        utr_exons = updated_exons[
                            (updated_exons['transcript_id'] == transcript_id) &
                            (updated_exons['end'] >= five_prime_utr_start) &
                            (updated_exons['start'] <= five_prime_utr_end)
                        ].copy()
                    else:
                        # updated_exons が空の場合はオリジナルの exons を使用
                        utr_exons = exons[
                            (exons['transcript_id'] == transcript_id) &
                            (exons['end'] >= five_prime_utr_start) &
                            (exons['start'] <= five_prime_utr_end)
                        ].copy()

                    # 各 exon に基づいて five_prime_UTR を作成
                    for idx, exon in utr_exons.iterrows():
                        utr_start = max(exon['start'], five_prime_utr_start)  # exon の開始位置と tss_pos の大きい方
                        utr_end = min(exon['end'], five_prime_utr_end)  # exon の終了位置と CDS の start - 1 の小さい方

                        if utr_start <= utr_end:  # 範囲が正しい場合のみ処理
                            if file_type == "gtf":
            ...
            # strand: '-' の処理
                ...
                if tss_pos > cds[cds['transcript_id'] == transcript_id]['end'].max():
                    ...
                    print(f"Updated gene end for gene_id {target_gene_id} to {tss_pos}.")
                    # 更新された gene 行を updated_gtf に追加
                    updated_gene = gene.iloc[0].to_dict()
                    updated_gene['original_index'] = gene.index[0]
                    updated_gtf.append(updated_gene)
                ...
                if five_prime_utr_start <= five_prime_utr_end:
                    ...
                    # 各 exon に基づいて five_prime_UTR を作成
                    for idx, exon in utr_exons.iterrows():
                        utr_start = max(exon['start'], five_prime_utr_start)  # exon の開始位置と CDS の end + 1 の大きい方
                        utr_end = min(exon['end'], five_prime_utr_end)  # exon の終了位置と tss_pos の小さい方
        ...
        # 複数のtranscript_idを持つ場合の処理
            ...
            print(f"Gene ID {target_gene_id} has multiple transcript_ids: {transcript_ids}")
            tss_pos = tss_group['tss_pos'].iloc[0]
            # strandを取得
            strand = gene.iloc[0]['strand']
            ...
            for transcript_id in transcript_ids:
                ...
                # 最小値を取得
                closest_transcripts = []  # 初期化
                if tss_to_exon_distance:  # 辞書が空でない場合のみ処理を実行
                    ...
                    for transcript_id in closest_transcripts:
                        print(f"Processing transcript_id {transcript_id} for gene_id {target_gene_id}")
                        # 選ばれた transcript_id の情報を取得
                        transcript_cds = cds[cds['transcript_id'] == transcript_id]
                        # 選択された transcript_id の start & end を取得 (mRNA, transcript feature が含まれないgtfに対応するためexonから取得)
                        transcript_start = exons[exons['transcript_id'] == transcript_id]['start'].min()
                        transcript_end = exons[exons['transcript_id'] == transcript_id]['end'].max()
                        ...
                        if strand == '+':
                            if tss_pos < cds[cds['transcript_id'] == transcript_id]['start'].min():
                                ...
                                if transcript_start == gene.iloc[0]['start']:
                                    ...
                                    updated_gene['original_index'] = gene.index[0]
                                    updated_gtf.append(updated_gene)
                                    print(f"Updated gene start for gene_id {target_gene_id} to {tss_pos} (matched transcript start).")
                        ...
                        # strand == '-' の場合の処理
                            ...
                            if tss_pos >= cds[cds['transcript_id'] == transcript_id]['end'].max():
                                ...
                                if transcript_end == gene.iloc[0]['end']:  
                                    ...
                                    gene.iloc[0, gene.columns.get_loc('end')] = new_gene_end
                                    print(f"Updated gene end for gene_id {target_gene_id} to {tss_pos} (matched transcript end).")
                                    # 更新された gene 行を updated_gtf に追加
                                    updated_gene = gene.iloc[0].to_dict()
                                    updated_gene['original_index'] = gene.index[0]
                                    updated_gtf.append(updated_gene)
                            ...
                            # tss_pos が CDS の end よりも大きい場合、TSS feature を追加
                            if tss_pos > cds[cds['transcript_id'] == transcript_id]['end'].max():
                                # TSS feature の追加処理
                                if not tss_group.empty:
                                    # TSS feature の属性を作成
    ...
    try:
        ...
        try:
            ...
            missing_features = ['gene', 'mRNA', 'transcript', 'exon']
